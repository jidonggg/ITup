...
**문제**: /about 등 주요 페이지가 AI가 생성한 것 같은 제네릭한 느낌
**요청**: "너무 ai 페이지 같아 조금 더 따뜻하고 사람이 쓴 것처럼 해"

**커밋**: `9d2c9ac` (4개 파일, +338줄, -390줄)
**배포**: Vercel 자동 배포
...
| 파일 | 변경 내용 |
|------|-----------|
| about/page.tsx | 제네릭 기업 가치관(실전 중심, 신뢰와 연결...) + 플레이스홀더 타임라인 → 스토리텔링 방식으로 전면 개편. "왜 만들었나" 섹션 추가, 구체적인 서비스 소개, 간결한 이용방법 |
| business/page.tsx | 과도한 그라데이션/이모지 제거, 서비스 카드를 깔끔한 텍스트 기반으로 정리, 폼 UI 간결화 (rounded-2xl → rounded-xl, 여백 정리) |
| faq/page.tsx | 자체 헤더 → 공통 Header/Footer 컴포넌트 적용, FAQ 데이터 간결화 (중복 질문 병합), 콘텐츠 톤 통일 |
| Stats.tsx | 가짜 소셜 프루프 숫자(1240+ 세션, 85명 멘토, 4.9/5.0, 73% 재이용률) 삭제 → 실제 서비스 정보(가격, 직군, 형태, 멘토 경력) 표시 |
