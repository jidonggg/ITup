...
CREATE POLICY "View page views" ON public.page_views FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "View events" ON public.analytics_events FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "View sessions" ON public.sessions FOR SELECT USING (auth.role() = 'authenticated');
